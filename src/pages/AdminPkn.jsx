...
import { 
  ...
  Upload, Loader2, CheckCircle2, Link2, HelpCircle, X,
  AlertCircle, BookOpen, HelpCircle as QuestionIcon
} from 'lucide-react';
import { supabase } from '../lib/supabaseClient';

export default function AdminPkn({ onBack }) {
  ...
  useEffect(() => {
    const handlePopState = (event) => {
      if (event.state && event.state.activePknTopic) {
        setActiveTopic(event.state.activePknTopic);
        setPdfUrl(event.state.activePknTopic.pdfUrl || '');
        setStorySlides(event.state.activePknTopic.storySlides || []);
        setMatchPairs(event.state.activePknTopic.matchPairs || []);
        setQuizList(event.state.activePknTopic.quiz || []);
      } else {
        setActiveTopic(null);
      }
    };
  ...
  const fetchModules = async () => {
    ...
    try {
      ...
      console.error('Error fetching PKn admin data:', err);
      showAlert('Gagal Ambil Data', 'Tidak dapat mengambil data modul PKn.', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateTopic = async (e) => {
    ...
    if (!newTopicHeader.judul) {
      return showAlert('Peringatan', 'Judul pertemuan wajib diisi!', 'warning');
    }
    ...
    try {
      ...
      showAlert('Berhasil!', 'Pertemuan PKn baru berhasil ditambahkan.', 'success');
      setNewTopicHeader({ 
  ...
  const handlePdfUpload = async (e) => {
    ...
    try {
      ...
      showAlert('Upload Berhasil', 'File PDF modul PKn berhasil diunggah!', 'success');
    } catch (err) {
  ...
  const handleOpenDetail = (topic) => {
    window.history.pushState({ view: 'adminDashboard', adminMapel: 'pkn', activePknTopic: topic }, '', '');
    setActiveTopic(topic);
    setPdfUrl(topic.pdfUrl || '');
    setStorySlides(topic.storySlides || []);
    setMatchPairs(topic.matchPairs || []);
    setQuizList(topic.quiz || []);
  };

  const handleAddSlide = () => {
    if (!newSlide.judul || !newSlide.teks) {
      return showAlert('Input Belum Lengkap', 'Judul slide dan isi konsep materi harus diisi!', 'warning');
    }
    setStorySlides([
      ...storySlides,
  ...
  const handleAddPair = () => {
    if (!newPair.left || !newPair.right) {
      return showAlert('Input Belum Lengkap', 'Isi pernyataan di sebelah kiri dan pasangannya di sebelah kanan!', 'warning');
    }
    setMatchPairs([
      ...matchPairs,
      { id: Date.now(), left: newPair.left.trim(), right: newPair.right.trim() }
    ]);
  ...
  const handleAddQuiz = () => {
    if (!newQuiz.soal || !newQuiz.optA || !newQuiz.optB || !newQuiz.jawabanBenar) {
      return showAlert('Input Belum Lengkap', 'Isi soal, pilihan jawaban, dan kunci jawaban!', 'warning');
    }
  ...
  const handleSaveMedia = async (e) => {
    ...
    try {
      ...
      showAlert('Tersimpan!', `Materi PKn "${activeTopic.judul}" berhasil disimpan!`, 'success');
      fetchModules();
    } catch (err) {
  ...
  const handleDeleteTopic = (id) => {
    showConfirm(
      'Hapus Pertemuan?',
      'Apakah kamu yakin ingin menghapus topik PKn ini secara permanen?',
      async () => {
        try {
          const { error } = await supabase.from('pkn_modules').delete().eq('id', id);
          if (error) throw error;
  ...
  if (loading) {
    return (
      <div className="w-full h-screen flex flex-col items-center justify-center space-y-3 bg-[#FAFAF8]">
        ...
        <p className="text-xs font-black text-zinc-600">Memuat Data Admin PKn...</p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full min-h-screen bg-[#FAFAF8] p-4 md:p-8 space-y-6 font-sans relative">
          ...
          <div className="bg-white border-3 border-zinc-950 rounded-3xl p-6 max-w-sm w-full shadow-[8px_8px_0px_0px_rgba(24,24,27,1)] space-y-4">
            <div className="flex items-start gap-3">
              <div className={`p-3 rounded-2xl border-2 border-zinc-950 shrink-0 ${
                modal.status === 'success' ? 'bg-emerald-300' : modal.status === 'error' ? 'bg-rose-300' : 'bg-purple-300'
              }`}>
                {modal.status === 'success' && <CheckCircle2 className="w-6 h-6 text-zinc-950" />}
                {modal.status === 'error' && <AlertCircle className="w-6 h-6 text-zinc-950" />}
                {modal.status === 'warning' && <QuestionIcon className="w-6 h-6 text-zinc-950" />}
                {modal.status === 'info' && <AlertCircle className="w-6 h-6 text-zinc-950" />}
              </div>
            ...
            <div className="flex items-center justify-end gap-2 pt-2 border-t-2 border-zinc-100">
              {modal.type === 'confirm' ? (
                <>
                  <button onClick={closeModal} className="flex-1 bg-white font-black py-2.5 rounded-xl border-2 border-zinc-950 text-xs shadow-[2px_2px_0px_0px_rgba(24,24,27,1)]">Batal</button>
                  <button onClick={() => { if (modal.onConfirm) modal.onConfirm(); closeModal(); }} className="flex-1 bg-rose-400 font-black py-2.5 rounded-xl border-2 border-zinc-950 text-xs shadow-[2px_2px_0px_0px_rgba(24,24,27,1)]">Ya, Lanjutkan</button>
                </>
              ) : (
                <button onClick={closeModal} className="w-full bg-purple-300 font-black py-2.5 rounded-xl border-2 border-zinc-950 text-xs shadow-[2px_2px_0px_0px_rgba(24,24,27,1)]">Mengerti</button>
              )}
            </div>
          </div>
        </div>
      ...
      <div className="bg-zinc-900 text-white border-2 border-zinc-900 rounded-3xl p-6 shadow-[5px_5px_0px_0px_rgba(24,24,27,1)]">
        <span className="bg-purple-300 text-zinc-950 text-[10px] font-black px-2.5 py-0.5 rounded border border-zinc-900 uppercase">ADMIN MAPEL</span>
        <h2 className="text-2xl md:text-3xl font-black mt-1 text-purple-300">Kelola Pendidikan Kewarganegaraan (PKn)</h2>
      </div>

      {!activeTopic ? (
        <div className="space-y-6">
          <button onClick={onBack} className="bg-white hover:bg-zinc-100 text-zinc-900 font-black px-4 py-2 rounded-xl border-2 border-zinc-900 shadow-[2px_2px_0px_0px_rgba(24,24,27,1)] flex items-center gap-2 text-xs">
          ...
          <div className="bg-white border-2 border-zinc-900 rounded-3xl p-6 shadow-[5px_5px_0px_0px_rgba(24,24,27,1)] space-y-4">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b-2 border-zinc-200 pb-4">
              <div>
                <h3 className="font-black text-lg text-zinc-900 flex items-center gap-2">
                  ...
                  <span>Daftar Pertemuan PKn</span>
                </h3>
                <p className="text-xs text-zinc-500 font-bold mt-0.5">
                  Kelola materi, slide cerita, game pasang-pasangan, dan kuis.
                </p>
              </div>
              ...
              <button 
                ...
                className="w-full sm:w-auto bg-purple-400 hover:bg-purple-500 text-zinc-950 font-black px-4 py-2.5 rounded-xl border-2 border-zinc-900 shadow-[2px_2px_0px_0px_rgba(24,24,27,1)] text-xs flex items-center justify-center gap-2 active:translate-x-0.5 active:translate-y-0.5 transition shrink-0"
              >
            ...
            {}
            {isAddingNew && (
              <form onSubmit={handleCreateTopic} className="bg-purple-50 border-2 border-zinc-900 p-4 rounded-2xl space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs font-bold">
                  ...
                  <input type="text" placeholder="Judul Topik (misal: Hak dan Kewajiban Warga Negara)" value={newTopicHeader.judul} onChange={(e) => setNewTopicHeader({ ...newTopicHeader, judul: e.target.value })} className="p-3 rounded-xl border-2 border-zinc-900 bg-white" />
                </div>
                ...
                <button type="submit" className="bg-zinc-900 text-purple-300 font-black px-4 py-2 rounded-xl border-2 border-zinc-900 text-xs shadow-[2px_2px_0px_0px_rgba(24,24,27,1)] active:translate-x-0.5 active:translate-y-0.5 transition">Simpan Pertemuan Ke Database</button>
              </form>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
              {modules.length === 0 ? (
                <div className="col-span-full py-10 text-center text-xs font-bold text-zinc-400">
                  Belum ada pertemuan PKn yang dibuat. Klik tombol di atas untuk menambah.
                </div>
              ) : (
                modules.map((item) => (
                  <div key={item.id} className="bg-zinc-50 border-2 border-zinc-900 rounded-2xl p-5 shadow-[3px_3px_0px_0px_rgba(24,24,27,1)] flex flex-col justify-between space-y-4">
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="bg-purple-200 text-purple-950 font-black text-[9px] px-2.5 py-0.5 rounded border border-zinc-900 uppercase">{item.pertemuan}</span>
                      ...
                      <div className="flex flex-wrap gap-1.5 pt-2 text-[10px] font-black">
                        ...
                        <span className={`px-2 py-0.5 rounded border ${item.matchPairs?.length ? 'bg-purple-100 text-purple-900 border-purple-300' : 'bg-zinc-200 text-zinc-500 border-zinc-300'}`}>{item.matchPairs?.length ? `✓ ${item.matchPairs.length} Pasang Kartu` : '✗ Pasangan'}</span>
                    ...
                    <button onClick={() => handleOpenDetail(item)} className="w-full bg-purple-300 hover:bg-purple-400 text-zinc-950 font-black p-3 rounded-xl border-2 border-zinc-900 text-xs flex items-center justify-center gap-2 shadow-[2px_2px_0px_0px_rgba(24,24,27,1)] active:translate-x-0.5 active:translate-y-0.5 transition">Kelola / Edit Isi Materi ➔</button>
                  </div>
                ))
              )}
            </div>
        ...
        <div className="space-y-6">
          <button onClick={() => window.history.back()} className="bg-white hover:bg-zinc-100 text-zinc-900 font-black px-4 py-2 rounded-xl border-2 border-zinc-900 shadow-[2px_2px_0px_0px_rgba(24,24,27,1)] flex items-center gap-2 text-xs">
          ...
          <div className="bg-white border-2 border-zinc-900 rounded-3xl p-6 shadow-[5px_5px_0px_0px_rgba(24,24,27,1)] space-y-8">
            <div className="border-b-2 border-zinc-200 pb-3">
              <span className="bg-purple-200 text-purple-950 font-black text-[10px] px-2.5 py-0.5 rounded border border-zinc-900 uppercase">{activeTopic.pertemuan}</span>
            ...
            <div className="space-y-3 bg-zinc-50 border-2 border-zinc-900 p-5 rounded-2xl">
              <label className="text-zinc-900 font-black flex items-center gap-2 text-sm">
                <FileText className="w-5 h-5 text-purple-600" /> Upload Modul PDF
              </label>

              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
                <label className="w-full sm:w-auto bg-white hover:bg-purple-50 border-2 border-dashed border-purple-500 rounded-xl px-5 py-3 cursor-pointer flex items-center justify-center gap-2 text-xs font-black text-purple-950 transition shadow-[1px_1px_0px_0px_rgba(24,24,27,1)]">
                  {uploadingPdf ? <Loader2 className="w-4 h-4 animate-spin text-purple-600" /> : <Upload className="w-4 h-4 text-purple-600" />}
            ...
            <div className="space-y-4 bg-zinc-50 border-2 border-zinc-900 p-5 rounded-2xl">
              <div className="flex items-center justify-between border-b-2 border-zinc-200 pb-2">
                <label className="text-zinc-900 font-black flex items-center gap-2 text-sm">
                  <BookOpen className="w-5 h-5 text-blue-600" /> Pojok Bacaan & Konsep Teori (Slide Cerita)
                </label>
                ...
                {storySlides.map((slide, idx) => (
                  <div key={slide.id} className="relative bg-white border-2 border-zinc-900 rounded-2xl p-4 shadow-[2px_2px_0px_0px_rgba(24,24,27,1)] flex items-start justify-between gap-4">
                    <div className="space-y-2 flex-1">
                      ...
                      <p className="text-xs font-bold text-zinc-700 leading-relaxed">"{slide.teks}"</p>
                    </div>
              ...
              <div className="bg-blue-50/50 border-2 border-blue-200 p-4 rounded-2xl space-y-3 text-xs font-bold">
                <h4 className="font-black text-blue-950 uppercase text-[11px]">+ Tambah Slide Cerita</h4>
                <input type="text" placeholder="Judul Slide (misal: Arti Hak & Kewajiban)" value={newSlide.judul} onChange={(e) => setNewSlide({ ...newSlide, judul: e.target.value })} className="w-full p-3 rounded-xl border-2 border-zinc-900 bg-white" />
                <textarea rows="3" placeholder="Isi penjelasan konsep materi..." value={newSlide.teks} onChange={(e) => setNewSlide({ ...newSlide, teks: e.target.value })} className="w-full p-3 rounded-xl border-2 border-zinc-900 bg-white resize-none" />
                <button type="button" onClick={handleAddSlide} className="bg-blue-400 hover:bg-blue-500 text-zinc-950 font-black py-2.5 px-4 rounded-xl border-2 border-zinc-900 shadow-[2px_2px_0px_0px_rgba(24,24,27,1)] uppercase text-xs active:translate-x-0.5 active:translate-y-0.5 transition">+ Simpan Slide Bacaan</button>
              </div>
            </div>
            ...
            <div className="space-y-4 bg-zinc-50 border-2 border-zinc-900 p-5 rounded-2xl">
              <div className="flex items-center justify-between border-b-2 border-zinc-200 pb-2">
                <label className="text-zinc-900 font-black flex items-center gap-2 text-sm">
                  <Link2 className="w-5 h-5 text-purple-600" /> Game Tarik Garis / Pasang-Pasangan
                </label>
                <span className="text-xs font-black text-zinc-500">{matchPairs.length} Pasang Kartu</span>
              </div>

              <div className="space-y-3">
                {matchPairs.map((pair, idx) => (
                  <div key={pair.id} className="relative bg-white border-2 border-zinc-900 rounded-2xl p-4 shadow-[2px_2px_0px_0px_rgba(24,24,27,1)] flex items-center justify-between gap-3">
                    <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-3 items-center">
                      <div className="p-3 bg-purple-50 rounded-xl border border-purple-300 text-xs font-bold text-zinc-900">
                        <span className="text-[10px] font-black text-purple-800 uppercase block mb-0.5">Kiri (Pernyataan/Soal #{idx + 1}):</span>
                        "{pair.left}"
                      </div>
                      <div className="p-3 bg-emerald-50 rounded-xl border border-emerald-300 text-xs font-bold text-zinc-900">
                        <span className="text-[10px] font-black text-emerald-800 uppercase block mb-0.5">Kanan (Pasangan Cocok):</span>
                        "{pair.right}"
                      </div>
                    </div>
                    <button type="button" onClick={() => handleDeletePair(pair.id)} className="bg-rose-50 hover:bg-rose-100 text-rose-600 p-2.5 rounded-xl border border-rose-300 shrink-0 transition">
                      <Trash2 className="w-4 h-4" />
                    </button>
                  </div>
                ))}
              ...
              <div className="bg-purple-50/60 border-2 border-purple-200 p-4 md:p-5 rounded-2xl space-y-3 text-xs font-bold">
                <h4 className="font-black text-purple-950 uppercase text-[11px]">+ Tambah Pasangan Kartu Baru</h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <label className="text-[10px] font-black text-purple-900 uppercase block mb-1">Teks Sebelah Kiri (Soal / Tindakan):</label>
                    <input 
                      type="text" 
                      placeholder="Contoh: Menjaga fasilitas sekolah" 
                      value={newPair.left} 
                      onChange={(e) => setNewPair({ ...newPair, left: e.target.value })} 
                      className="w-full p-3 rounded-xl border-2 border-zinc-900 bg-white font-bold text-xs" 
                    />
                  </div>
                  <div>
                    <label className="text-[10px] font-black text-purple-900 uppercase block mb-1">Teks Sebelah Kanan (Pasangannya):</label>
                    <input 
                      type="text" 
                      placeholder="Contoh: Kewajiban di Sekolah" 
                      value={newPair.right} 
                ...
                <button 
                  ...
                  className="w-full bg-purple-300 hover:bg-purple-400 text-zinc-950 font-black py-3 rounded-xl border-2 border-zinc-900 shadow-[2px_2px_0px_0px_rgba(24,24,27,1)] uppercase text-xs active:translate-x-0.5 active:translate-y-0.5 transition flex items-center justify-center gap-1.5"
                >
                  <Plus className="w-4 h-4" /> Simpan Pasangan Kartu
                </button>
              </div>
            </div>
            ...
            <div className="space-y-4 bg-zinc-50 border-2 border-zinc-900 p-5 rounded-2xl">
              <div className="flex items-center justify-between border-b-2 border-zinc-200 pb-2">
                <label className="text-zinc-900 font-black flex items-center gap-2 text-sm">
                  <HelpCircle className="w-5 h-5 text-amber-500" /> Mini Quiz Pemahaman PKn
                </label>
              ...
              <div className="bg-amber-50/60 border-2 border-amber-300 p-4 rounded-2xl space-y-3 text-xs font-bold pt-3">
                ...
                <div>
                  ...
                  <input 
                    type="text" 
                    placeholder="Sesuatu yang mutlak menjadi milik seseorang dan penggunaannya tergantung pada diri sendiri disebut..." 
                    value={newQuiz.soal} 
                ...
                <div>
                  ...
                  <input 
                    type="text" 
                    placeholder="Tuliskan Persis Jawaban Yang Benar (Misal: Hak)" 
                ...
                <button 
                  ...
                  className="w-full bg-purple-300 hover:bg-purple-400 text-zinc-950 font-black py-2.5 rounded-xl border-2 border-zinc-900 uppercase text-xs active:translate-x-0.5 active:translate-y-0.5 transition shadow-[2px_2px_0px_0px_rgba(24,24,27,1)]"
                >
