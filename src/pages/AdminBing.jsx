...
export default function AdminBing({ onBack }) {
  ...
  useEffect(() => {
    const handlePopState = (event) => {
      if (event.state && event.state.activeBingTopic) {
        setActiveTopic(event.state.activeBingTopic);
        setPdfUrl(event.state.activeBingTopic.pdfUrl || '');
        setFlashcards(event.state.activeBingTopic.flashcards || []);
        setQuizList(event.state.activeBingTopic.quiz || []);
      } else {
        setActiveTopic(null);
      }
    };
  ...
  const handlePdfUpload = async (e) => {
    ...
    try {
      ...
      const filePath = `english/${fileName}`;
  ...
  const handleOpenDetail = (topic) => {
    window.history.pushState({ view: 'adminDashboard', adminMapel: 'english', activeBingTopic: topic }, '', '');
  ...
  const handleAddFlashcard = () => {
    if (!newFlashcard.idTeks || !newFlashcard.enTeks) {
      return showAlert('Input Belum Lengkap', 'Isi teks Indonesia dan English terlebih dahulu!', 'warning');
    }
  ...
  const handleDeleteTopic = (id) => {
    showConfirm(
      'Hapus Pertemuan?',
      'Apakah kamu yakin ingin menghapus topik/pertemuan ini beserta isinya secara permanen?',
      async () => {
        try {
          const { error } = await supabase
            .from('english_modules')
  ...
  if (loading) {
    return (
      <div className="w-full h-screen flex flex-col items-center justify-center space-y-3 bg-[#FAFAF8]">
        ...
        <p className="text-xs font-black text-zinc-600">Memuat Data Admin English is Fun!...</p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full min-h-screen bg-[#FAFAF8] p-4 md:p-8 space-y-6 font-sans relative">
          ...
          <div className="bg-white border-3 border-zinc-950 rounded-3xl p-6 max-w-sm w-full shadow-[8px_8px_0px_0px_rgba(24,24,27,1)] space-y-4 animate-in zoom-in-95 duration-150">
            ...
            <div className="flex items-center justify-end gap-2 pt-2 border-t-2 border-zinc-100">
              {modal.type === 'confirm' ? (
                ...
                <button
                  onClick={closeModal}
                  className="w-full bg-emerald-400 hover:bg-emerald-500 text-zinc-950 font-black py-2.5 rounded-xl border-2 border-zinc-950 text-xs shadow-[2px_2px_0px_0px_rgba(24,24,27,1)] active:translate-x-0.5 active:translate-y-0.5 transition"
                >
                  Mengerti
                </button>
              )}
      ...
      <div className="bg-zinc-900 text-white border-2 border-zinc-900 rounded-3xl p-6 shadow-[5px_5px_0px_0px_rgba(24,24,27,1)]">
        <span className="bg-emerald-400 text-zinc-950 text-[10px] font-black px-2.5 py-0.5 rounded border border-zinc-900 uppercase">
          ADMIN MAPEL
        </span>
        <h2 className="text-2xl md:text-3xl font-black mt-1 text-emerald-400">
          Kelola English is Fun! (Bahasa Inggris)
        </h2>
      </div>
      ...
      {!activeTopic ? (
        <div className="space-y-6">
          ...
          <div className="bg-white border-2 border-zinc-900 rounded-3xl p-6 shadow-[5px_5px_0px_0px_rgba(24,24,27,1)] space-y-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b-2 border-zinc-200 pb-4">
              <div>
                <h3 className="font-black text-lg text-zinc-900 flex items-center gap-2">
                  ...
                  <span>Daftar Pertemuan English is Fun!</span>
                </h3>
                <p className="text-xs text-zinc-500 font-bold mt-0.5">
                  Pilih pertemuan di bawah untuk mengedit modul PDF, flashcard, dan kuis.
                </p>
              </div>
              ...
              <button 
                ...
                className="w-full sm:w-auto bg-emerald-400 hover:bg-emerald-500 text-zinc-950 font-black px-4 py-2.5 rounded-xl border-2 border-zinc-900 shadow-[2px_2px_0px_0px_rgba(24,24,27,1)] text-xs flex items-center justify-center gap-2 active:translate-x-0.5 active:translate-y-0.5 transition shrink-0"
              >
            ...
            {}
            {isAddingNew && (
              <form onSubmit={handleCreateTopic} className="bg-emerald-50 border-2 border-zinc-900 p-4 rounded-2xl space-y-3">
                <h4 className="font-black text-xs uppercase text-emerald-950">Buat Wadah Pertemuan Baru</h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs font-bold">
                  <input 
                    type="text" 
                    placeholder="Sesi (misal: Pertemuan 2)" 
                  ...
                  <input 
                    type="text" 
                    placeholder="Judul Topik (misal: Numbers & Colors)" 
                ...
                >
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="bg-emerald-300 text-zinc-950 font-black text-[9px] px-2.5 py-0.5 rounded border border-zinc-900 uppercase">
                        {item.pertemuan}
                      </span>
                      <button 
                    ...
                    <div className="flex flex-wrap gap-1.5 pt-2 text-[10px] font-black">
                      ...
                      <span className={`px-2 py-0.5 rounded border ${item.flashcards?.length ? 'bg-yellow-100 text-yellow-900 border-yellow-300' : 'bg-zinc-200 text-zinc-500 border-zinc-300'}`}>
                        {item.flashcards?.length ? `✓ ${item.flashcards.length} Flashcard` : '✗ Flashcard'}
                      </span>
                  ...
                  <button 
                    ...
                    className="w-full bg-emerald-400 hover:bg-emerald-500 text-zinc-950 font-black p-3 rounded-xl border-2 border-zinc-900 text-xs flex items-center justify-center gap-2 shadow-[2px_2px_0px_0px_rgba(24,24,27,1)] active:translate-x-0.5 active:translate-y-0.5 transition"
                  >
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border-2 border-zinc-900 rounded-3xl p-6 shadow-[5px_5px_0px_0px_rgba(24,24,27,1)] space-y-8">
            <div className="border-b-2 border-zinc-200 pb-3">
              <span className="bg-emerald-300 text-zinc-950 font-black text-[10px] px-2.5 py-0.5 rounded border border-zinc-900 uppercase">
            ...
            <div className="space-y-3 bg-zinc-50 border-2 border-zinc-900 p-5 rounded-2xl">
              <label className="text-zinc-900 font-black flex items-center gap-2 text-sm">
                <FileText className="w-5 h-5 text-emerald-600" /> Upload Modul PDF
              </label>

              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
                <label className="w-full sm:w-auto bg-white hover:bg-emerald-50 border-2 border-dashed border-emerald-500 rounded-xl px-5 py-3 cursor-pointer flex items-center justify-center gap-2 text-xs font-black text-emerald-950 transition shadow-[1px_1px_0px_0px_rgba(24,24,27,1)]">
                  {uploadingPdf ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
                {flashcards.map((card) => (
                  <div key={card.id} className="relative bg-white border-2 border-zinc-900 rounded-2xl p-4 shadow-[3px_3px_0px_0px_rgba(24,24,27,1)] flex flex-col items-center justify-center space-y-1 min-h-[120px]">
                    ...
                    <span className="text-[9px] font-black text-zinc-400 uppercase tracking-wider">Indonesia</span>
                    ...
                    <span className="text-[9px] font-black text-emerald-600 uppercase tracking-wider">English</span>
                    <span className="font-black text-xs text-zinc-900 text-center uppercase">{card.enTeks}</span>
                  </div>
                ))}
            ...
            <div className="space-y-4 bg-zinc-50 border-2 border-zinc-900 p-5 rounded-2xl">
              ...
              <div className="bg-amber-50/60 border-2 border-amber-300 p-4 rounded-2xl space-y-3 text-xs font-bold pt-3">
                ...
                <div>
                  ...
                  <input 
                    type="text" 
                    placeholder="Bahasa Inggris dari 'Kepala' adalah..." 
                    value={newQuiz.soal} 
                ...
                <div>
                  ...
                  <input 
                    type="text" 
                    placeholder="Tuliskan persis jawaban yang benar (misal: Head)" 
                ...
                <button 
                  ...
                  className="w-full bg-amber-400 hover:bg-amber-500 text-zinc-950 font-black py-2.5 rounded-xl border-2 border-zinc-900 shadow-[2px_2px_0px_0px_rgba(24,24,27,1)] uppercase text-xs active:translate-x-0.5 active:translate-y-0.5 transition"
                >
